...
def priority(n):
    ...
    while clock <= n:
        ...
        if index <= len(df["Process_ID"])-1 and clock == df["Arrival_Time"][index]:
            ...
            if len(taskQueue) > 0 and df["Priority"][index] > taskQueue[0][2]:
                prevTask = taskQueue[0]
                taskQueue.insert(0, [df["Process_ID"][index], df["Execution_Time"][index], df["Priority"][index]])
                numSusProcesses+=1
                susFlag = True
                ...
                taskQueue.append([df["Process_ID"][index], df["Execution_Time"][index], df["Priority"][index]])
        ...
        if len(taskQueue) > 0:
            ...
            if currentTask[1]==0:
                ...
                # Increasing priority by one of every element in taskQueue (Ageing)
                taskQueue = age(taskQueue)
    ...
    df1.to_csv("graphData4.csv", index=False)
